...
Related Coursework Section: "Double DQN with Memory Replay"
...
****** SHOULD SCORE 500 POINTS 75% OF THE TIME WITHIN 300 EPOCHS ******
"""
import gym
import random
import torch
...
class Shallow_Q_Learner(RLDebugger):
    def __init__(self, state_shape, action_shape, learning_rate=0.003, gamma=0.8):
        RLDebugger.__init__(self)
        self.state_shape = state_shape
        self.action_shape = action_shape
        self.gamma = gamma #Discount Factor
        self.learning_rate = learning_rate  # learning rate
        self.best_mean_reward = - float("inf") # Best Mean Reward
        self.best_reward = - float("inf") #Best epoch reward
        self.training_steps_completed = 0  # Number of training batch steps completed so far
        
        self.Q = SLP(state_shape, action_shape) #DQN for observations, the DQN we will use gradient descent upon
        self.Q_target = SLP(state_shape, action_shape) #DQN for selecting actions, will have Q copied over it afer every epoch
        ...
        self.epsilon_decay = 0.999561
        
        self.step_num = 0
        self.memory = ExperienceMemory()  # Initialise an Experience memory with 1000 capacity
    ...
    def learn_from_batch_experience(self, experiences):
        ...
        td_target = reward_batch + ~done_batch * \
                ...
                self.Q_target(next_obs_batch).detach().max(1)[0].data.cpu().numpy()
    ...
    def learn(self, obs, action, reward, next_obs, done):
        ...
        if done:  # End of episode
            td_target = self.Q_target(next_obs) * 0.0 + reward
        else:
            td_target = reward + self.gamma * torch.max(self.Q_target(next_obs))
    ...
    for episode in range(MAX_NUM_EPISODES):
        ...
        while not done:
            ...
            if done is True:
                ...
                # copy weights from the DQN model used for action selection 
                # to the DQN model used for computing targets
                agent.Q_target.load_state_dict(agent.Q.state_dict())
                
                this_step = step+1
                print("\nEpisode#{} ended in  {} steps. reward ={} mean_reward={} best_reward={} epsilon={}".
                      format(episode, this_step, cum_reward, np.mean(episode_rewards), agent.best_reward, agent.epsilon))
                new_result = []
                ...
                new_result.append(agent.epsilon)
